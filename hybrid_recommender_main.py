...
from hybrid_recommender_module import create_item_embedding_matrix, create_user_embedding_matrix, ModelBasedCollaborativeFiltering, create_dataset_info, train_model, MemoryBasedCollaborativeFiltering, create_rating_matrix, HybridRecommender
...
def main():
    ...
    train_user_mapped_idx = map_ids_to_idx_users(train_user_embeddings)
    val_user_mapped_idx = map_ids_to_idx_users(val_user_embeddings)
    ...
    val_interactions_data = create_dataset_info(foodcom_interaction_val_df, val_user_mapped_idx, recipe_mapped_idx)
    ...
    model = HybridRecommender(content_model, collab_model, 0.5)
    model.to(device)

    # Train model
    train_model(model, device, train_interactions_data, val_interactions_data, 256, True, 100)
